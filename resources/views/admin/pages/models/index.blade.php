...
<div style="display:block; margin-top: 30px">
    <h1>الموديلات</h1>
    <a href="{{route('models.create')}}" class="btn btn-primary mt-5">
        <i class="ion ion-plus"></i> اضافة موديل
    </a>
</div>
        ...
        <tbody>
            @foreach ($models as $item)
                <tr data-widget="expandable-table" aria-expanded="false">
                    ...
                    <td>{{ $item->name_en }}</td>
                    <td>
                        <a href="{{route('models.edit', $item->id)}}">
                            <i class="fa fa-edit"></i>
                        </a>
                        <form action="{{route('models.destroy', $item->id)}}" class="d-inline-block"  method="post">
                            @csrf
                            @method('DELETE')
