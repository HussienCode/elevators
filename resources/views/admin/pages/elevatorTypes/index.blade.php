...
<div style="display:block; margin-top: 30px">
    <h1>انواع المصاعد</h1>
    <a href="{{route('elevatorTypes.create')}}" class="btn btn-primary mt-5">
        <i class="ion ion-plus"></i> اضافة نوع مصعد جديد
    </a>
</div>
        ...
        <tbody>
            @foreach ($elevatorTypes as $item)
                <tr data-widget="expandable-table" aria-expanded="false">
                    ...
                    <td>{{ $item->name_en }}</td>
                    <td>
                        <a href="{{route('elevatorTypes.edit', $item->id)}}">
                            <i class="fa fa-edit"></i>
                        </a>
                        <form action="{{route('elevatorTypes.destroy', $item->id)}}" class="d-inline-block"  method="post">
                            @csrf
                            @method('DELETE')
