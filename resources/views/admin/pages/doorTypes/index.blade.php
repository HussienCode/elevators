...
<div style="display:block; margin-top: 30px">
    <h1>انواع الابواب</h1>
    <a href="{{route('doorTypes.create')}}" class="btn btn-primary mt-5">
        <i class="ion ion-plus"></i> اضافة نوع جديد للأبواب
    </a>
</div>
        ...
        <tbody>
            @foreach ($doorTypes as $item)
                <tr data-widget="expandable-table" aria-expanded="false">
                    ...
                    <td>{{ $item->name_en }}</td>
                    <td>
                        <a href="{{route('doorTypes.edit', $item->id)}}">
                            <i class="fa fa-edit"></i>
                        </a>
                        <form action="{{route('doorTypes.destroy', $item->id)}}" class="d-inline-block"  method="post">
                            @csrf
                            @method('DELETE')
